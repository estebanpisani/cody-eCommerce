...
const userControllers = {
  signUp: async (req, res) => {
    const { firstName, lastName, email, password, image, method } =
      req.body.userData;
    try {
      ...
      if (userExists) {
        ...
        if (method !== "signUpForm") {
          (newUser.verification = true), await newUser.save();
          res.json({
            success: true,
            from: "Google",
            message:
              "Felicitaciones, tu cuenta fue creada con " +
              from +
          ...
          res.json({
            ...
            from: method,
            message:
              "Te enviamos un correo para verificar tu cuenta, mira tu casilla y segui los pasos",
          });
        }
      }
    } catch (error) {
        ...
        if (from === "signUpForm") {
          if (loginUser.verification) {
            if (checkedWord.length >= 0) {
              ...
              const token = jwt.sign({ id: loginUser._id }, ck.SECRET_KEY, {
  ...
  unsubscribeUser: async (req, res) => {
    ...
    try {
      user = await User.findOneAndUpdate({ _id: id }, unsuscribe, {
        new: true,
      });
    ...
    res.json({
      response: error
        ? "ERROR"
        : "Tu usuario ha sido eliminado de la base de datos.",
